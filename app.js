...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('mow-lawn-button').addEventListener('click', mowLawn);

    // Function for each prompt to show in terminal (changed all previous alerts to div element with text)
    function newPromptTextToTerminal(text) {
        const newPromptDiv = document.createElement('div');
        const newPromptText = document.createTextNode(text);
        newPromptDiv.appendChild(newPromptText);
        document.querySelector('.terminal-prompt-container').appendChild(newPromptDiv);
    };

    // Player clicks button "New Game", alert shows first action
    function startGame() {
        currentMoney = landscaper.money += currentPayRate;
        currentPayRate = landscaper.payRate[0];
        newPromptTextToTerminal(`You used your ${currentTool} to mow the lawns and made $${currentPayRate}.00 today!`);
        document.getElementById("currentTool").innerHTML = currentTool;
        document.getElementById("currentMoney").innerHTML = currentMoney;
        newPromptTextToTerminal(`It's a new day! Click the "Mow Lawn" button to earn some more $$$`);
        // console.log(landscaper.tools)
        document.getElementById('mow-lawn-button').removeAttribute('disabled');
    };

    // Function to continue mowing the lawn every day
    function mowLawn() {
        currentMoney += currentPayRate;
        newPromptTextToTerminal(`You mowed the lawn today with your ${currentTool} and earned $${currentPayRate}.00 more!`);
        document.getElementById("currentTool").innerHTML = currentTool;
        document.getElementById("currentMoney").innerHTML = currentMoney;
        document.getElementById("currentPayRate").innerHTML = currentPayRate;
        buyScissors()
    ...
    function endGame() {
        if (currentMoney >= 1000 && currentTool === toolInventory.name[4]) {
            ...
            newPromptTextToTerminal(`CONGRATS! YOU WON THE GAME!`);
        };
    };
